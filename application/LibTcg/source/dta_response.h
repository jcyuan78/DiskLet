...
public:
    ...
    * @param tokenNum the 0 based number of the token*/
    uint64_t getUint64(uint32_t tokenNum) const;
    ...
    * @param tokenNum the 0 based number of the token*/
    uint8_t getUint8(uint32_t tokenNum) const;
    ...
    * @param bytearray pointer to array for return data */
    ...
    OPALHeader m_header; /**< TCG Header fields of the response */

public:
    BYTE GetMethodStatus(void) const { return getUint8(getTokenCount() - 4); }
    const wchar_t* GetMetholdStatusString(void)
    {
        return MethodStatusCodeToString(GetMethodStatus());
    }
