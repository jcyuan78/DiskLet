...
typedef BYTE HUID[4];       // half uid;

inline void HiHalfUid(HUID& out, const TCG_UID& in)
{
    memcpy_s(out, 4, in, 4);
}

inline void LoHalfUid(HUID& out, const TCG_UID& in)
{
    memcpy_s(out, 4, in + 4, 4);
}

inline void MakeUid(TCG_UID& out, const HUID& hi, const HUID& lo)
{
    memcpy_s(out, 4, hi, 4);
    memcpy_s(out + 4, 4, lo, 4);
}
...
namespace tcg
{
    namespace opal_sp
    {   // SPs
        const TCG_UID SMUID =       { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff }; // session management
        const TCG_UID THISSP =      { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01 }; // special "thisSP" syntax
        const TCG_UID ADMINSP =     { 0x00, 0x00, 0x02, 0x05, 0x00, 0x00, 0x00, 0x01 }; // Administrative SP
        const TCG_UID LOCKINGSP =   { 0x00, 0x00, 0x02, 0x05, 0x00, 0x00, 0x00, 0x02 }; // Locking SP 
    };
    namespace opal_authority
    {   // Authority
        const TCG_UID TEMPLATE=     { 0x00, 0x00, 0x00, 0x09, 0x00, 0x00, 0x00, 0x00 };
        const TCG_UID ANYBODY =     { 0x00, 0x00, 0x00, 0x09, 0x00, 0x00, 0x00, 0x01 }; /**<anybody */
        const TCG_UID ADMINS =      { 0x00, 0x00, 0x00, 0x09, 0x00, 0x00, 0x00, 0x02 }; /**< ADMIN1 */
        const TCG_UID SID =         { 0x00, 0x00, 0x00, 0x09, 0x00, 0x00, 0x00, 0x06 }; // SID
        const TCG_UID ADMIN1 =      { 0x00, 0x00, 0x00, 0x09, 0x00, 0x01, 0x00, 0x01 }; /**< ADMIN1 */
        const TCG_UID USERS =       { 0x00, 0x00, 0x00, 0x09, 0x00, 0x03, 0x00, 0x00 }; /**< USER1 */
        const TCG_UID USER1 =       { 0x00, 0x00, 0x00, 0x09, 0x00, 0x03, 0x00, 0x01 }; /**< USER1 */
    };
    namespace opal_obj
    {
        const TCG_UID C_PIN_MSID =      { 0x00, 0x00, 0x00, 0x0B, 0x00, 0x00, 0x84, 0x02 }; // C_PIN_MSID 
        const TCG_UID C_PIN_SID =       { 0x00, 0x00, 0x00, 0x0B, 0x00, 0x00, 0x00, 0x01 }; // C_PIN_SID 
        const TCG_UID DATASTORE =       { 0x00, 0x00, 0x10, 0x01, 0x00, 0x00, 0x00, 0x00 }; // Datastore Tab
        const TCG_UID LOCKING_INFO =    { 0x00, 0x00, 0x08, 0x01, 0x00, 0x00, 0x00, 0x01 }; // Locking info (obj tab)
        const TCG_UID LOCKING_GLOBAL =  { 0x00, 0x00, 0x08, 0x02, 0x00, 0x00, 0x00, 0x01 }; //Locking_GlobalRange
        const TCG_UID LOCKING_RANGE =   { 0x00, 0x00, 0x08, 0x02, 0x00, 0x03, 0x00, 0x00 }; //Locking Range
    }
    namespace opal_ace
    {
        const TCG_UID DATASTORE_READ =  { 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0xFC, 0x00 }; // ACE: Read Datastore 
...
const TCG_UID OPAL_PSID_UID =           { 0x00, 0x00, 0x00, 0x09, 0x00, 0x01, 0xff, 0x01 }; /**< PSID user */
//const TCG_UID OPAL_ ={ 0x00, 0x00, 0x00, 0x09, 0x00, 0x00, 0x80, 0x01 }; /**< BandMaster 0 */
//const TCG_UID OPAL_ ={ 0x00, 0x00, 0x00, 0x09, 0x00, 0x00, 0x84, 0x01 }; /**< EraseMaster */
//const TCG_UID OPAL_ = tables                                           ;
//const TCG_UID OPAL_ ={ 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0xE0, 0x01 }; /**< ACE_Locking_Range_Set_RdLocked UID */
//const TCG_UID OPAL_ ={ 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0xE8, 0x01 }; /**< ACE_Locking_Range_Set_WrLocked UID */
//const TCG_UID OPAL_ ={ 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0xE0, 0x00 }; /**< ACE_Locking_Range_GLOBAL_Set_RdLocked UID */                                                                     ;
//const TCG_UID OPAL_ ={ 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0xE8, 0x00 }; /**< ACE_Locking_Range_GLOBAL_Set_WrLocked UID */                                                                     ;
//const TCG_UID OPAL_ ={ 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0xF8, 0x01 }; /**< ACE_MBRControl_Set_DoneToDOR */
const TCG_UID OPAL_MBR_CONTROL =        { 0x00, 0x00, 0x08, 0x03, 0x00, 0x00, 0x00, 0x01 }; // MBR Control (object table)
const TCG_UID OPAL_SHADOW_MBR =         { 0x00, 0x00, 0x08, 0x04, 0x00, 0x00, 0x00, 0x00 }; // Shadow MBR (byte table)
const TCG_UID OPAL_AUTHORITY_TABLE =    { 0x00, 0x00, 0x00, 0x09, 0x00, 0x00, 0x00, 0x00 }; // AUTHORITY_TABLE
const TCG_UID OPAL_AUTHORITY_USER =     { 0x00, 0x00, 0x00, 0x09, 0x00, 0x03, 0x00, 0x00 }; // USERS in AUTHORITY_TABLE
...
const TCG_UID ACE_LOCKING_RANGE_SET_RD = { 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0xE0, 0x00 };
const TCG_UID ACE_LOCKING_RANGE_SET_WR = { 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0xE8, 0x00 };
...
const TCG_UID METHOD_GENKEY =           { 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, 0x10 }; //GenKey 
...
const TCG_UID TYPE_AUTHORITY_OBJ = { 0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x0C, 0x05 };
const HUID HTYPE_AUTHORITY_OBJ = { 0x00, 0x00, 0x0C, 0x05 };
const HUID HTYPE_BOOLEAN_ACE = { 0x00, 0x00, 0x04, 0x0E };
enum BOOLEAN_TYPE { BOOLEAN_AND=0, BOOLEAN_OR=1,BOOLEAN_NOT=2,};
//GUID 
...
/** Useful tiny atoms. Useful for table columns etc */
